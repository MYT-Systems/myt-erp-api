...
EOT;
        ...
        } elseif ($payment_status === 'recurring') {
            // Auto-generated by Cron::generate_pending_recurring_invoices(), which always
            // inserts with added_by = 0 (system user) — manual invoices always have a real user id.
            $sql .= ' AND project_invoice.status = "pending" AND project_invoice.added_by = 0';
        } elseif ($payment_status) {
            $sql .= ' AND project_invoice.payment_status = ?';
            $binds[] = $payment_status;
        } elseif (empty($payment_status)) {
            $sql .= ' AND project_invoice.status = "pending" AND (project_invoice.added_by != 0 OR project_invoice.added_by IS NULL)';
        }
