...
class Project_invoices extends MYTController
{
    ...
    protected function _attempt_update_project_invoice_items($project_invoice, $db)
    {
        ...
        if ($project_invoice['fs_status'] == 'invoiced' && !$this->_revert_project_item($project_invoice['id'], $seller_branch_id, $buyer_branch_id)) {
        ...
        if ($project_invoice['fs_status'] != 'quoted' && !$new_credit = $this->_restore_credit_limit($project_invoice)) {
        ...
        if ($project_invoice['fs_status'] != 'quoted' && $new_credit < $grand_total && $old_grand_total != $grand_total) {
    ...
    {
        $seller_branch_id = $project_invoice['seller_branch_id'];
        $buyer_branch_id  = $project_invoice['buyer_branch_id'];
        ...
        if ($project_invoice['fs_status'] == 'invoiced' && !$this->_revert_project_item($project_invoice['id'], $seller_branch_id, $buyer_branch_id)) {
            ...
            var_dump("failed to delete project invoice item model");
            return false;
        } elseif (($project_invoice['fs_status'] == 'invoiced' || $project_invoice['fs_status'] == 'processing') && 
                !$this->_restore_credit_limit($project_invoice)) {
            var_dump("failed to restore credit limit");
            return false;
        }
        ...
        switch ($status) {
            ...
            case 'processing':
                ...
                if ($project_invoice['fs_status'] == 'invoiced') {
                    ...
                    if (!$this->_revert_project_item($project_invoice['id'], $seller_branch_id, $buyer_branch_id)) {
                        var_dump("failed to revert and delete");
                        return false;
                    }
                }
