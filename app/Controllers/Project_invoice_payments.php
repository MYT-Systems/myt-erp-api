...
class Project_invoice_payments extends MYTController
{
    ...
    public function get_project_invoice_payment()
    {
        ...
        $project_invoice_payment    = $project_invoice_payment_id ? $this->projectInvoiceSalePaymentModel->get_details_by_id($project_invoice_payment_id) : null;
        $project_invoice            = $project_invoice_payment ? $this->projectInvoiceSaleModel->get_details_by_id($project_invoice_payment[0]['project_invoice_id']) : null;
    ...
    public function get_all_project_invoice_payment()
    {
        ...
        $project_invoice_payments = $this->projectInvoiceSalePaymentModel->get_all();

        if (!$project_invoice_payments) {
            $response = $this->failNotFound('No project invoice sale payment found');
        } else {
            foreach ($project_invoice_payments as $key => $project_invoice_payment) {
                $project_invoice = $this->projectInvoiceSaleModel->get_details_by_id($project_invoice_payment['project_invoice_id']);
    ...
    public function update($id = null)
    {
        ...
        if (!$project_invoice_payment = $this->projectInvoiceSalePaymentModel->select('', $where, 1)) {
    ...
    public function search()
    {
        ...
        if (!$project_invoice_payments = $this->projectInvoiceSalePaymentModel->search($project_id, $project_invoice_id, $payment_method, $payment_date_from, $payment_date_to, $from_bank_id, $cheque_number, $cheque_date_from, $cheque_date_to, $reference_number, $transaction_number, $branch_name)) {
            $response = $this->failNotFound('No project invoice sale payment found');
        } else {
            foreach ($project_invoice_payments as $key => $project_invoice_payment) {
                $project_invoice_payments[$key]['project_invoice'] = $this->projectInvoiceSaleModel->get_details_by_id($project_invoice_payment['project_invoice_id']);
            }
    ...
    {
        $project_invoice_items = $this->projectInvoiceSaleItemModel->get_details_by_project_invoices_id($project_invoice['id']);
    ...
    {        
        if ($project_invoice = $this->projectInvoiceSaleModel->get_details_by_id($this->request->getVar('project_invoice_id'))) {
            $project_invoice = $project_invoice[0];
        } else {
            $this->errorMessage = 'Invalid project invoice sale id';
            return false;
        }
        ...
        if (!$project_invoice_payment_id = $this->projectInvoiceSalePaymentModel->insert($values)) {
            $this->errorMessage = $this->db->error()['message'];
           return false;
        }
    ...
    protected function _attempt_update($project_invoice_payment_id)
    {
        ...
        $project_invoice_payment = $this->projectInvoiceSalePaymentModel->get_details_by_id($project_invoice_payment_id);
        $project_invoice_payment = $project_invoice_payment[0];
        $project_invoice_id = $project_invoice_payment['project_invoice_id'];
        $project_invoice = $this->projectInvoiceSaleModel->get_details_by_id($project_invoice_id);
        $project_invoice = $project_invoice[0];
        ...
        if (!$this->projectInvoiceSaleModel->update($project_invoice_id, $new_values)) {
            $this->errorMessage = $this->db->error()['message'];
            return false;
        }
        ...
        if (!$this->projectInvoiceSalePaymentModel->update($project_invoice_payment_id, $values)) {
            $this->errorMessage = $this->db->error()['message'];
            return false;
        }

        if (!$project_invoice = $this->projectInvoiceSaleModel->get_details_by_id($values['project_invoice_id'])) {
            $this->errorMessage = $this->db->error()['message'];
            return false;
        }
    ...
    protected function _record_sale_payment($project_invoice, $values) {
        ...
        if (!$this->projectInvoiceSaleModel->update($project_invoice['id'], $update_values)) {
            $this->errorMessage = $this->db->error()['message'];
            return false;
        }
    ...
    protected function _attempt_delete($project_invoice_payment)
    {
        ...
        if (!$this->projectInvoiceSalePaymentModel->update($project_invoice_payment['id'], $values)) {
            $this->errorMessage = $this->db->error()['message'];
            return false;
        }
        
        // Restore the credit limit back
        $project_invoice = $this->projectInvoiceSaleModel->get_details_by_id($project_invoice_payment['project_invoice_id'])[0];
        ...
        if (!$this->projectInvoiceSaleModel->update($project_invoice['id'], $new_values)) {
            $this->errorMessage = $this->db->error()['message'];
            return false;
        }
    ...
    {
        $this->projectInvoiceSalePaymentModel = model('App\Models\Project_invoice_payment');
        $this->projectInvoiceSaleModel        = model('App\Models\Project_invoice');
        $this->projectInvoiceSaleItemModel    = model('App\Models\Project_invoice_item');
